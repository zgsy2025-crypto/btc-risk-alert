...
def kline(symbol: str, interval: str, limit: int = 200) -> List[Dict[str, float]]:
...
def check_signals() -> List[tuple[str, str]]:
    candles_1m = kline(SYMBOL, "1m", 120)
    candles_1h = kline(SYMBOL, "1h", 220)

    closes_1m = [candle["close"] for candle in candles_1m]
    volumes_1m = [candle["volume"] for candle in candles_1m]
    ...
    for minutes, key in windows:
        if len(closes_1m) > minutes:
            change = pct_change(closes_1m[-minutes - 1], last_price)
            ...
            if abs(change) >= threshold:
                ...
                body = (
                    f"현재가: <b>{fmt_price(last_price)} USDT</b>\n"
                    f"{minutes}분 변동률: <b>{change:.2f}%</b>\n\n"
                    f"해석: 단기 변동성이 커졌습니다. 선물 포지션은 손절가·청산가·포지션 크기를 먼저 확인하세요."
                )
                alerts.append((f"{key}_{'up' if change > 0 else 'down'}", make_alert(title, body)))

    if len(volumes_1m) >= 25:
        avg20 = sum(volumes_1m[-21:-1]) / 20
        latest_volume = volumes_1m[-1]
        multiple = latest_volume / avg20 if avg20 else 0

        if multiple >= THRESHOLDS["volume_spike_mult"]:
            title = "BTC 1분봉 거래량 폭증"
            body = (
                f"현재가: <b>{fmt_price(last_price)} USDT</b>\n"
                f"최근 거래량: 평균 대비 <b>{multiple:.1f}배</b>\n\n"
    ...
    if ema200_1h is not None and rsi_1h is not None:
        if last_price < ema200_1h and rsi_1h < THRESHOLDS["trend_risk_rsi"]:
...
def main() -> None:
    ...
    # 최초 연결 확인용 메시지입니다.
    # 정상 작동 확인 후 매 5분마다 테스트 메시지가 오는 것이 싫으면 아래 한 줄을 삭제하세요.
